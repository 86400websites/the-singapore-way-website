...
export default function LessonSidebar({
  ...
  return (
    <>
      {/* Mobile: compact, collapsible curriculum shown above the lesson.
          Native <details> keeps this a Server Component with no client JS. */}
      <details className="group lg:hidden rounded-2xl border border-[#ECECEC] bg-white overflow-hidden">
        <summary className="focus-ring list-none cursor-pointer select-none px-5 py-4 [&::-webkit-details-marker]:hidden">
          <span className="flex items-center justify-between gap-4">
            <span className="eyebrow">Course menu</span>
            <span className="flex-shrink-0 inline-flex items-center gap-1.5 text-[12px] font-bold tracking-[0.08em] uppercase text-[#666666]">
              <span className="group-open:hidden">Show lessons</span>
              <span className="hidden group-open:inline">Hide lessons</span>
              <svg
          ...
          {showProgress ? (
            <span className="mt-3 block">
              <span className="flex items-center justify-between text-[12px] text-[#444444]">
              ...
              <span
                className="mt-2 block h-1.5 rounded-full bg-[#ECECEC] overflow-hidden"
                aria-hidden="true"
              >
                <span
                  className="block h-full bg-[#0a8553]"
                  style={{ width: `${progress.percent}%` }}
                />
              </span>
            </span>
          ) : (
            <span className="mt-2 block text-[13px] text-[#666666]">
              {course.modules.length} modules · {totalLessons} lessons
            </span>
          )}
        </summary>

        <div className="border-t border-[#F0F0F0] px-5 py-5">
          {showProgress && progress.percent >= 100 && (
            <Link
              href={`/courses/${course.slug}/certificate`}
              className="focus-ring rounded-sm mb-5 inline-flex items-center gap-2 text-[12px] font-bold tracking-[0.08em] uppercase text-[#0a8553] hover:underline"
            >
      ...
      >
        ...
        <CurriculumNav
          ...
          activeLessonSlug={activeLessonSlug}
          completedLessonIds={completedLessonIds}
          className="lg:max-h-[calc(100vh-260px)] lg:overflow-y-auto lg:pr-2"
        />
      </aside>
    </>
  )
