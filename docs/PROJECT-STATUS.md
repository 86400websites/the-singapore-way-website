...
| Active sprint | S13 — Online Course Finalisation — Status: In Progress (Phases 1–4 implemented; DB package authored, awaiting owner apply + Preview QA + Codex review) |
| Current branch | `claude/s13-online-course-finalisation` |
| Next action | 1. Open PR from `claude/s13-online-course-finalisation` → Vercel Preview. 2. Full Preview QA per the S13 pack (journeys, quiz boundaries, certificate print/verify, a11y, Lighthouse). 3. Codex review of the immutable range → owner merge → Production smoke. — *Done:* owner applied `0006` to the shared project (2026-07-23); builder verified via read-only MCP: 5 modules / 16 video / 5 quiz / 21 required, 25 questions (per-quiz key checksums match the approved bank), all 16 tracker URLs mapped in order, certificate preserved (1), progress/attempts reset (0/0). |
