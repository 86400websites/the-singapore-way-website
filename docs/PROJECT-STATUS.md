...
| Active sprint | S13 — Online Course Finalisation — Status: In Progress (all phases implemented; 0006 + 0007 applied by owner and MCP-verified; Codex rounds 1–2 = REQUEST CHANGES, all code/doc fixes landed incl. 0008 — awaiting owner 0008 apply, PR + Preview QA evidence, Codex round 3) |
| Current branch | `claude/s13-online-course-finalisation` |
| Next action | 1. Owner hand-applies `supabase/sql/0008_certificate_public_name_gate.sql` (required — Codex round-2 fix); builder verifies via read-only MCP (boolean-only checks: both functions gated, 0 non-displayable certificates). 2. **Open the PR** from `claude/s13-online-course-finalisation` → Vercel Preview → full pack-08 QA at the exact head, record URL/tester/date. 3. Codex round 3 with the brief fully filled → owner merge → Production smoke. — *Done:* `0006` + `0007` applied 2026-07-23 and MCP-verified (curriculum 5/16/5/21 + 25 questions with matching key checksums; name gate live; the sole remaining certificate is backed by full completion of the final curriculum, issued 2026-07-23). Codex round 1 (`b41c657`) and round 2 (`c7f93c0`): REQUEST CHANGES; all code/doc fixes landed (record: [`code-reviews/S13-online-course-finalisation-review.md`](./code-reviews/S13-online-course-finalisation-review.md)). |
