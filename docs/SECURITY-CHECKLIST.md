...
- [ ] 🔴 Every gated route checks the session (and any role/approval flag) **server-side**. File location is not access control. — *Verify: open each gated route — `/courses/[slug]/learn/[lessonSlug]`, `/courses/[slug]/certificate`, `/my-learning`, `/account` — and find the explicit `supabase.auth.getUser()` check with redirect to `/login`. (`/learn` is a public marketing hub, not a gated route — see [`./TECH-ARCHITECTURE.md`](./TECH-ARCHITECTURE.md) §3.)*
...
- [ ] 🔴 **Delivery dependencies fail CLOSED in Production.** If a required delivery key is missing, the form refuses with an honest error — it never silently drops data. — *Verify: missing Mailchimp/Resend keys throw `MissingServerEnvError` (`src/lib/server-env.ts`) and the handlers return an honest 503; the branch keys on key presence, not `NODE_ENV`. No-op delivery is acceptable only in local/Preview.*
- [ ] 🔴 **Abuse controls must be set in Vercel Production — they currently fail OPEN if unset.** `checkRateLimit` (`src/lib/rate-limit.ts:59`) returns `success` and `verifyTurnstileToken` (`src/lib/turnstile/verify.ts:23`) returns `ok` when their keys are absent, with **no `NODE_ENV`/`VERCEL_ENV` guard**, so a missing key silently disables the control in Production too. The guarantee is operational, not code-enforced. — *Verify: confirm `TURNSTILE_SECRET_KEY`, `UPSTASH_REDIS_REST_URL`, and `UPSTASH_REDIS_REST_TOKEN` are set in Vercel Production. Hardening the code to fail closed on absent abuse-control keys in Production is a recorded gap (see §9.3 and [`./PROJECT-STATUS.md`](./PROJECT-STATUS.md) §10).*
...
- [ ] 🔴 **Public writes are validated; delivery fails CLOSED; abuse controls are enforced operationally.** `/api/newsletter` and `/api/contact` (and the same pattern in `/api/mailchimp/subscribe`) validate with zod server-side, verify Turnstile server-side, and rate-limit per-IP via Upstash. Delivery fails closed (503) when a delivery key is missing. **The abuse controls fail OPEN when their keys are absent — the code has no Production guard** — so the blocking requirement is that `TURNSTILE_SECRET_KEY`, `UPSTASH_REDIS_REST_URL`, and `UPSTASH_REDIS_REST_TOKEN` are set in Vercel Production. *(set 2026-07-22)* — *Verify: read all three handlers per §5; confirm those three env vars are set in Vercel Production (nothing in the code refuses a submission if they are missing). Code-level fail-closed hardening for absent abuse-control keys is tracked as required-before-scale in [`./PROJECT-STATUS.md`](./PROJECT-STATUS.md) §10.*
