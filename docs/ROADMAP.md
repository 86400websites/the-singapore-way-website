...
| S12 | Add the `/close` and `/sprint-prompt` Claude Code skills under `.claude/skills/`, adapted to this repo and consistent with the governing docs | App code, config, CI, SQL; external SOP-pack changes; the abuse-control code fix | Both skills committed and repo-consistent (fail-open reality, no admin role, our conventions/templates); typecheck/lint/build unchanged | S11 | Done (merged 2026-07-23, PR #14) |
| S13 | Online Course Finalisation: replace the sample course with the approved 5-module / 16-YouTube-video / 5-quiz (25-question) course; real `youtube-nocookie` player + one-origin CSP addition; certificate name-gate, print, and approved redesign (typographic signature); `0006`–`0008` migrations (owner-applied by hand); read-only Supabase MCP tooling | Custom domain migration; CMS/admin; payments/roles; analytics expansion; auth replacement; PDF/QR dependencies; automatic SQL execution; fabricated durations; imitation signatures | 16 videos live + tracker-mapped; 5 quizzes server-graded at 80% with keys DB-only; approved copy shipped; certificate issues/prints/verifies behind the name gate; DB verified post-apply (16/5/21, 25, gates live) | S12 | Done (merged 2026-07-23, PR #15; 3 Codex rounds — all findings fixed; merged by owner without a recorded APPROVE, deviation noted in the sprint record) |
| S14 | Certificate template + verify-link fix: rebuild `CertificateView.tsx` to the new approved ornamental mockup (red corner motifs, flanked eyebrows, "proudly presented to" copy); fix the print-to-PDF verify-link truncation bug by rendering the URL as a real `<a>` (embeds a full-URL PDF link annotation) and keeping it one line in print. Also add the browser-tools verification layer to the repo: `docs/BROWSER-TOOLS.md` + `.claude/skills/browser-qa/` + `qa-evidence/`/`.playwright-mcp/` gitignore | Course content, copy facts, DB/RPC/routes, auth, the name gate; new fonts or binary assets (corners are inline SVG); PDF/QR libraries | Certificate matches the mockup at 320/768/1440 with no 320px overflow and prints on one A4 landscape page; the printed PDF carries a real link annotation with the complete `/certificates/<uuid>` URL (verified via headless print-to-PDF); browser-qa skill + doc present and repo-consistent; typecheck/lint/build pass | S13 | Done (merged 2026-07-24, PR #17; Codex round 1 REQUEST CHANGES on the evidence gate only — no code findings; merged by owner with deployed-Preview QA outstanding, compensated by local print-to-PDF proof + read-only Production smoke; deviation in the sprint record) |
