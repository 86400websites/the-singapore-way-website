...
| S13 | Online Course Finalisation: replace the sample course with the approved 5-module / 16-YouTube-video / 5-quiz (25-question) course; real `youtube-nocookie` player + one-origin CSP addition; certificate name-gate, print, and approved redesign (typographic signature); `0006` Path A content migration + rewritten seed (owner applies by hand); read-only Supabase MCP tooling | Custom domain migration; CMS/admin; payments/roles; analytics expansion; auth replacement; PDF/QR dependencies; automatic SQL execution; fabricated durations; imitation signatures | 16 videos live + tracker-mapped; 5 quizzes server-graded at 80% with keys DB-only; approved copy shipped; certificate issues/prints/verifies; DB counts verified post-apply (16/5/21, 25); full pack QA + Codex APPROVE + owner merge + Production smoke | S12 | In Progress |
