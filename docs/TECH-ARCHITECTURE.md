...
| `/certificates/[certificateId]` | Public certificate verification — safe fields only (id, course title, display name, issued date); returns not-found for a blank/generic-name owner (0008) | Public | Public shell | Supabase (`get_public_certificate` RPC) |
...
| `certificates` | Issued course certificates | The signed-in user (per-row) | Owner-scoped SELECT under RLS; public projection via `get_public_certificate` exposes only id, course title, display name, issued date — and returns no row for a blank/generic-name owner (0008; the email local-part is never used) | Server-only via `issue_certificate` RPC, which re-verifies completion and requires a meaningful full name (0007/0008) | Cascades on user deletion; verification page depends on row existing |
...
- [ ] Private files require authorized, short-lived delivery or an equivalent protected mechanism. — N/A — no private file storage today; course videos are YouTube embeds (`video_url` holds the approved tracker URL, served only via the sign-in-gated `get_signed_in_lesson_body` RPC and framed from `www.youtube-nocookie.com`) and all site images are public assets under `public/assets/`.
...
| Content types | Marketing/editorial page copy; typed blog posts; podcast episode metadata (Wix-hosted MP3s); Q&A; course curriculum (5 modules, 16 required YouTube video lessons, 5 quizzes of 5 questions at 80% pass, certificates); images under `public/assets/<feature>/`. |
...
- Security headers and transport controls are defined in [`../next.config.ts`](../next.config.ts) on `/:path*` and verified on the deployed response as part of Preview/launch QA: `Content-Security-Policy` (default-src 'self'; third-party origins allow-listed for Supabase, PostHog, Sentry, Cloudflare Turnstile, Wixstatic podcast media, the Railway-hosted Ideate iframe, and YouTube privacy-enhanced course embeds — `www.youtube-nocookie.com`, frame-src only; `frame-ancestors 'none'`; nonce-based CSP is a recorded follow-up since `'unsafe-inline'` is required today), `Strict-Transport-Security` (max-age 63072000; includeSubDomains; preload), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (camera/microphone/geolocation disabled).
