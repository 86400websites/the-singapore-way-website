...
| Upstash Redis (rate limiting) | No — but a required abuse-prevention control in Production (protects all three public API handlers) | No-op when absent (local/Preview); when configured, over-limit requests are refused with HTTP 429 + `Retry-After` | `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` | Client IP + rate counters |
| Cloudflare Turnstile (bot protection) | No — but a required abuse-prevention control in Production (public forms) | Widget hidden / verification skipped when unset (local/Preview); when configured, failed verification is refused with HTTP 401 | `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY` | Turnstile token, client IP |
...
Optional integrations may be disabled in local or Preview when documented — all of the above no-op when their env vars are absent, which is what lets CI build without secrets. In Production, a required delivery, payment, or abuse-prevention dependency fails closed with an honest error; it never reports success while losing work — the newsletter and contact handlers validate with zod server-side, verify Turnstile, rate-limit via Upstash, and refuse (not fake-success) when a required key is missing.
