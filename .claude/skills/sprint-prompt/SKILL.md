...
4. **Output, in this order:**
   ...
   - **F. Optional Codex review brief** — only for risky sprints (auth/gating, RLS/schema, env, security headers, CSP, public writes). Produce it by filling `docs/templates/CODEX-REVIEW-PROMPT-TEMPLATE.md`; it will be saved at `docs/code-reviews/[SPRINT_ID]-[SLUG]-review.md`. Otherwise say review is optional per WORKFLOW and skip it.
...
4. Commit it with the sprint's closing PR if still open; otherwise as a small dedicated `docs/` branch (PROJECT-STATUS §11 — tracker/record flips ride a tiny docs branch once the sprint branch is merged).
