---
name: browser-qa
description: Browser verification for The Singapore Way using the globally installed Playwright MCP and Agent Browser CLI. Use whenever work needs real-browser eyes - capturing visual QA evidence for a UI sprint (screenshots at 320/768/1440 plus states on the deployed Preview), testing forms and flows on Preview, responsive checks, console/network debugging, reproducing a reported UI bug, or self-capturing the Preview evidence during /close. Triggers - "take screenshots", "test the preview", "check the form", "visual QA", "capture evidence", "why does this look broken", "does it work on mobile", after any UI change before its PR, and whenever QA-CHECKLIST Part 2 evidence is required.
---

# Browser QA — real-browser verification (The Singapore Way)
...
- `docs/DESIGN.md` §7 (working rules), §5 (motion), §6 (accessibility) — what "looks right" means here.
...
4. Judge against the mockup and `docs/DESIGN.md` §7: report **PASS or the exact visual gaps** (file:element, what's wrong, which rule it breaks). "It renders" is not the bar.
5. Hand the operator the evidence list to attach/link in the PR beside the Preview record (`docs/templates/VERCEL-PREVIEW-TEST-TEMPLATE.md`).
...
- Report the reproduction steps and evidence; the fix itself belongs to the sprint/bug-fix loop (`docs/templates/BUG-FIX-PROMPT-TEMPLATE.md`), not this skill.

**D. Close-assist (`/close`)**
